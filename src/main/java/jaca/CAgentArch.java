...
public class CAgentArch extends AgArch implements cartago.ICartagoListener, Serializable {
    ...
    */
	...
	public void act(ActionExec actionExec) {
		...
		try {
			...
			if (!failed) {
				...
				/** PART TO BE FIXED RELATED TO NS **/
				/*
					...
					outer: for (ArtifactId aid1 : focusedArtifacts(action.getNS())) {// iterates artifacts focused using nsp associated with the action
						try {
							ArtifactInfo info = CartagoEnvironment.getInstance()
									.getController(aid1.getWorkspaceId().getName())
				...
				if (actId == -1) {
					...
					if (op.getName().equals("joinWorkspace")){
						...
						if (params.length > 0) {
							...
							URI uri = URI.create(wspRef);
							actId = envSession.joinWorkspace(uri);
						...
						if (params.length > 0) {
							...
							if (index == -1) {
								...
								if (!wspRef.startsWith("/")) {
									fullPath = currentImplicitWsp.getName() + "/" + wspRef;
								}
					...
					if (op.getName().equals("lookupArtifact") || op.getName().equals("makeArtifact")) {
						Object[] params = op.getParamValues();
						if (params.length > 0) {
							...
							/*
								...
								if (!artRef.startsWith("/")) {
									fullPath = currentImplicitWsp.getName() + "/" + artRef;
								}
	...
	/** TMP FIX 4.0 - not supporting NSP */
	...
	private void perceiveStopFocus(StopFocusSucceededEvent ev1) throws CartagoException, NoSuchFieldException, IllegalAccessException {
		...
		Atom nsp = Literal.DefaultNS; // ((NameSpaceOp) ev1.getOp()).getNS();
			...
			if (!aa.isEmpty()) {
				...
				for (AgentId ag : CartagoEnvironment.getInstance().getController(wspName).getCurrentAgents()) // to get the agentId from agName
					if (ag.getAgentName().equals(getAgName())) {
						...
						// 4.0 --- ignoring the target workspace - since we have a single wsp
						
						Workspace wsp = CartagoEnvironment.getInstance().getWorkspace();
	...
	*/
	...
	private void addSourceAnnots(ArtifactId source, Literal struct) throws Exception {
		...
		struct.addAnnot(ASSyntax.createStructure("workspace", ASSyntax.createString(source.getWorkspaceId().getName()), lib.objectToTerm(source.getWorkspaceId())));
	}
	...
	public void addObsPropertiesBel(ArtifactId source, ArtifactObsProperty prop, Atom nsp) {
		try {
			...
			/** TMP 4.0 FIX - no support for NSP */
			
			// nsp = processSpecialOP(source, prop, nsp);

			nsp = Literal.DefaultNS;
			if (nsp != null) {
	...
	/** Jason-related ASPECT TO BE FIXED GIVEN CArtAgO NEW VERSION -- BEGIN */
	...
	private Set<ArtifactId> computeFocusedArts() throws CartagoException {
    	...
    	for (WorkspaceId w: envSession.getJoinedWorkspaces()) {
    		ICartagoController ctrl = CartagoEnvironment.getInstance().getController(w.getName());
	...
	*/
	...
	/** Jason-related ASPECT TO BE FIXED GIVEN CArtAgO NEW VERSION -- END */
